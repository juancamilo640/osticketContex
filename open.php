...
function sendWebhookPayload($url, array $payload) {
...
function notifyTicketCreationWebhooks($ticket) {
    global $cfg;

    $urls = $cfg->get('ticket_webhook_urls');
    if (!$urls) {
        $urls = getenv('OSTICKET_TICKET_WEBHOOK_URLS') ?: getenv('TICKET_WEBHOOK_URLS');
    }
    if (!$urls) {
        return;
    }
    ...
    $payload = array(
        ...
        'status' => $ticket->getStatus() ? $ticket->getStatus()->getName() : null,
        'department' => $ticket->getDept() ? $ticket->getDept()->getName() : null,
        'topic' => $ticket->getTopic() ? $ticket->getTopic()->getName() : null,
        'url' => sprintf('%sview.php?id=%s', ROOT_PATH, urlencode($ticket->getId())),
        'source' => SOURCE,
        'created' => date('c'),
    );

    foreach ($urls as $url) {
        sendWebhookPayload($url, $payload);
    }
}
...
if ($_POST) {
    ...
    if(($ticket=Ticket::create($vars, $errors, SOURCE))){
        ...
        // Send webhook notifications if any external endpoints are configured
        notifyTicketCreationWebhooks($ticket);

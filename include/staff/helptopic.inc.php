...
if ($topic && $_REQUEST['a'] != 'add') {
    ...
    $title = __('Nuevo tema de ayuda');
    $action = 'create';
    $submit_text = __('Crear');
...
<ul class="clean tabs" id="topic-tabs">
    <li class="active"><a href="#info"><i class="icon-info-sign"></i>
            <?php echo __('Informacion del tema de ayuda'); ?></a></li>
...
<form action="helptopics.php?<?php echo Http::build_query($qs); ?>" method="post" class="save">
    ...
    <div id="topic-tabs_container">
        <div class="tab_content" id="info">
            <table class="table" border="0" cellspacing="0" cellpadding="2">
                <tbody>
                    <tr>
                        ...
                        <td>
                            <input type="text" size="30" name="topic" value="<?php echo $info['topic']; ?>" autofocus
                                data-translate-tag="<?php echo $trans['name']; ?>" />
                            &nbsp;<span class="error">*&nbsp;<?php echo $errors['topic']; ?></span> <i
                                class="help-tip icon-question-sign" href="#topic"></i>
                        </td>
                    </tr>
                    <tr>
                        <td width="180" class="required">
                        ...
                        <td>
                            <select name="status">
                                <option value="active" <?php echo ($info['status'] == __('Active')) ? 'selected="selected"' : ''; ?>>
                                    <?php echo __('Active'); ?></option>
                                <option value="disabled" <?php echo ($info['status'] == __('Disabled')) ? 'selected="selected"' : ''; ?>>
                                    <?php echo __('Disabled'); ?></option>
                                <option value="archived" <?php echo ($info['status'] == __('Archived')) ? 'selected="selected"' : ''; ?>>
                                    <?php echo __('Archived'); ?></option>
                            </select>
                            &nbsp;<span class="error">*&nbsp;<?php echo $errors['status']; ?></span> <i
                                class="help-tip icon-question-sign" href="#htstatus"></i>
                        </td>
                    </tr>
                    <tr>
                        <td width="180" class="required">
                        ...
                        <td>
                            ...
                            &nbsp;<span class="error">*&nbsp;<?php echo $errors['ispublic']; ?></span> <i
                                class="help-tip icon-question-sign" href="#type"></i>
                        </td>
                    </tr>
                    <tr>
                        <td width="180">
            ...
            <textarea class="richtext no-bar" name="notes" cols="21" rows="8"
                style="width: 80%;"><?php echo Format::viewableImages($info['notes']); ?></textarea>
        ...
        <div class="hidden tab_content" id="routing">
            ...
            <table class="table" border="0" cellspacing="0" cellpadding="2">
                        ...
                        <td>
                            <label>
                                <input type="radio" name="custom-numbers" value="0" <?php echo !$info['custom-numbers'] ? 'checked="checked"' : ''; ?>
                                    onchange="javascript:$('#custom-numbers').hide();">
                                <?php echo __('System Default'); ?>
                            </label>&nbsp;<label>
                ...
                <tbody id="custom-numbers" style="<?php if (!$info['custom-numbers'])
                    echo 'display:none'; ?>">
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="sla_id">
                                <option value="0">&mdash; <?php echo __("Departamento por defecto"); ?> &mdash;</option>
                                <?php
                                if ($slas = SLA::getSLAs()) {
                                    foreach ($slas as $id => $name) {
                                        echo sprintf(
                                            ...
                                            $id,
                                            ($info['sla_id'] == $id) ? 'selected="selected"' : '',
                                            $name
                                        );
                                    }
                                }
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="page_id">
                                <option value="">&mdash; <?php echo __('Por defecto'); ?> &mdash;</option>
                                <?php
                                if (($pages = Page::getActiveThankYouPages())) {
                                    foreach ($pages as $page) {
                                        if (strcasecmp($page->getType(), 'thank-you'))
                                            continue;
                                        echo sprintf(
                                            '<option value="%d" %s>%s</option>',
                                            $page->getId(),
                                ...
                                if ($teams = Team::getTeams()) { ?>
                                    ...
                                    echo sprintf(__('Teams (%d)'), count($teams)); ?>"><?php
                                       foreach ($teams as $id => $name) {
                                           ...
                                           ?>
                                            <option value="<?php echo $k; ?>" <?php echo $selected; ?>><?php echo $name; ?>
                                            </option>
                                            <?php
                                       } ?>
                                        <option value="0" data-quick-add data-id-prefix="t">—
                                            <?php echo __('Nuevo equipo'); ?> —</option>
                                    </optgroup>
                                    <?php
                                } ?>
                            </select>
        ...
        <div class="hidden tab_content" id="forms">
                ...
                return false;"><i class="icon-large icon-trash"></i></a>
                        ...
                        foreach ($F->getDynamicFields() as $f) { ?>
                            <tr>
                                ...
                                <td><?php $t = FormField::getFieldType($f->get('type'));
                                echo __($t[0]); ?></td>
            ...
            <select name="form_id" id="newform">
                ...
                <?php foreach (DynamicForm::objects()
                    ...
                    as $F) { ?>
                    <option value="<?php echo $F->get('id'); ?>" <?php if (in_array($F->id, $current_forms))
                           echo 'disabled="disabled"'; ?>     <?php if ($F->get('id') == $info['form_id'])
    ...
    <p style="text-align:center;">
        ...
        <input type="button" name="cancel" value="<?php echo __('Cancel'); ?>"
            onclick='window.location.href="helptopics.php"'>
    </p>
</form>
